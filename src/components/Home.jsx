...
function Home() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const currentUser = await account.get(); // Check if there's a logged-in user

      if (currentUser) {
        await account.deleteSessions(); // Log out any existing session
      }
    } catch (error) {
      // Handle error or assume no user is logged in
      console.log('No existing session found or error fetching user:', error);
    }
